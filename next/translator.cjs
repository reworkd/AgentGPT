...
class Translator {
  constructor(srcLang, targetLang, transService, settings = {}) {
    ...
    this.openaiTranslationMethod =
      settings.openaiTranslationMethod || process.env.OPENAI_TRANSLATION_METHOD || "chat";

    if (this.translatorService === "openai" && process.env.OPENAI_API_KEY !== "") {
  ...
  extractKeyNamespacePairs(filePath) {
    ...
    const regex = /(?:translate|i18n?.t|t)\("([^"]+)"(?:, "([^"]+)")(?:, "([^"]+)")?\)/g;
    let match;

    while ((match = regex.exec(fileContent)) !== null) {
      const key = match[1];
      const defaultText = match[3] ? match[2] : match[2] !== "" ? match[2] : "common";
  ...
  async translateAndWriteFiles() {
    ...
    if (this.keyTextNamespacePairs.length > 0) {
      await Promise.all(
        this.keyTextNamespacePairs.map(async ({ key, defaultText, namespace }) => {
          const targetLangFilePath = path.join(translationDir, `${namespace}.json`);
          if (!fs.existsSync(targetLangFilePath)) {
            fs.writeFileSync(targetLangFilePath, "{}");
          }
  ...
  async translateText(sourceTranslationValue) {
    if (this.translatorService === "google") {
      return await this.GoogleTranslate(this.srcLang, this.targetLang, sourceTranslationValue);
    } else if (this.translatorService === "openai" && this.openaiTranslationMethod === "chat") {
      return await this.translateViaChatCompletion(sourceTranslationValue);
    } else if (this.translatorService === "openai" && this.openaiTranslationMethod === "text") {
  ...
  // Services
  // GoogleTranslate
  GoogleTranslate = async (sourceLanguageCode, targetLanguageCode, sourceTranslationValue) => {
  ...
  async run(srcDirectory) {
    ...
    // insert here function which walks through on each file in the translation directory and checks for missing or null values in the JSON files if any of the values are missing, it prompts the user to enter the translation for the missing value and writes the updated JSON file to the translation directory
