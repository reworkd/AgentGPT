          ...
          <>
            ...
            <Expand delay={0.9} type="spring">
              <ChatMessage
                message={{
                  type: MESSAGE_TYPE_SYSTEM,
                  value: `${t("YOU_CAN_PROVIDE_YOUR_API_KEY", "YOU_CAN_PROVIDE_YOUR_API_KEY", { ns: "chat" })}`,
                }}
              />
              {showDonation && (
  ...
  const copyElementText = (elementId: string) => {
    ...
    if (navigator.clipboard) {
      ...
      try {
        ...
        console.log(`${i18n?.t("CONSOLE_TEXT_COPIED_TO_CLIPBOARD", "TEXT_COPIED_TO_CLIPBOARD", { ns: "chat" })}`);
      } catch (err) {
        console.error(`${i18n?.t("CONSOLE_UNABLE_TO_COPY_TO_CLIPBOARD", "CONSOLE_UNABLE_TO_COPY_TO_CLIPBOARD", { ns: "chat" })}`, err);
      }
    ...
    >
      ...
      {message.type == MESSAGE_TYPE_THINKING && (
        <span className="italic text-zinc-400">
          {`${t("RESTART_IF_IT_TAKES_X_SEC", "RESTART_IF_IT_TAKES_X_SEC", { ns: "chat" })}`}
        </span>
      )}
  ...
  return (
    <div className="mx-2 my-1 flex flex-col gap-2 rounded-lg border-[2px] border-white/10 bg-blue-500/20 p-1 text-center font-mono hover:border-[#1E88E5]/40 sm:mx-4 sm:p-3 sm:text-base md:flex-row">
      <div className="max-w-none flex-grow">
        {t("HELP_SUPPORT_THE_ADVANCEMENT_OF_AGENTGPT", "HELP_SUPPORT_THE_ADVANCEMENT_OF_AGENTGPT", { ns: "chat" })}
        <br />
        {t("CONSIDER_SPONSORING_ON_GITHUB", "CONSIDER_SPONSORING_ON_GITHUB", { ns: "chat" })}
      </div>
...
const getMessagePrefix = (message: Message, t: Translation) => {
  if (message.type === MESSAGE_TYPE_GOAL) {
    return `${i18n?.t("EMBARKING_ON_NEW_GOAL", "EMBARKING_ON_NEW_GOAL", { ns: "chat" })}`;
    ...
    return `${i18n?.t("COMPLETING", "COMPLETING", { ns: "chat" })} ${message.value}`;
