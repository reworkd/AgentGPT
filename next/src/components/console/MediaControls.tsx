...
export const MediaControls = ({ status, ...props }: MediaControlsProps) => {
  ...
  const disableOnLoading = loading && "cursor-not-allowed border-white/10 text-white/30";
    ...
    <AnimatePresence>
      ...
      >
        ...
        {status != "stopped" && (
          <motion.button
            className={clsx(
              "flex w-10 -translate-x-2 flex-row justify-center rounded-r-full border-2 border-white/50 bg-amber-500 py-2 transition-all hover:border-white/25 hover:bg-amber-400",
              disableOnLoading
            )}
            initial={{ x: -40, y: 0, opacity: 0 }}
            exit={{ x: -40, y: 0 }}
